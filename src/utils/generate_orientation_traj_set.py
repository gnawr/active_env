...
def generate_orientation_traj_set(feat_list):
	...
	starts = np.array([pick3, pick1])*(math.pi/180.0) # upright, upside down
	goals = np.array([place0, place2])*(math.pi/180.0) # upright, upside down
	goal_pose = None
	T = 20.0
	timestep = 0.5
	...
	print 'Expected number of trajs per start', len(object_centers_dict) * len(goals) * len(weights_list)
	...
	for start_idx, start in enumerate(starts):
		...
		here = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '../../'))
		savefile = "/data/traj_sets/start" + str(start_idx) + "traj_set_fixed_envs_human_coffee_laptop.p"
	...
	file_path = os.path.join(os.getcwd(), 'time_0304.txt')
